...
export function StudioScreen() {
  ...
  const onDrop = useCallback((p: DragPayload, res: Extract<DropResolution, { ok: true }>) => {
    if (p.cosmetic.category === 'remover') {
      ...
      if (target) {
        ...
        playSound('clear');
        stageRef.current?.bounce();
        setHint({ text: S.hintRemoved, mood: 'good' });
      } else {
        playSound('boing');
  ...
  const renderPhotoFor = useCallback((bg: PhotoBg): HTMLCanvasElement => {
    const blank = makeCanvas(512, 512);
    if (!compositor || !face) return renderPhoto(blank, bg, '', '');
    // Το compositor.out είναι κοινός καμβάς → αντίγραφο πριν αποδοθεί ξανά το στάδιο.
    const src = compositor.render('smile', layersRef.current);
    const copy = makeCanvas(512, 512);
    ctx2d(copy).drawImage(src, 0, 0);
    stageRef.current?.render(exprRef.current, layersRef.current);
    const who = session.modelName ? `${face.nameEl} · ${session.modelName}` : face.nameEl;
    return renderPhoto(copy, bg, who, S.appTitle);
  }, [compositor, face, session.modelName]);

  const onPhotoDone = (r: ShareResult) => {
    if (r === 'shared') setToast(S.photoShared);
    else if (r === 'downloaded') setToast(S.photoDownloaded);
    else if (r === 'failed') setToast(S.photoFailed);
    if (r !== 'cancelled') setDialog('none');
  };
  ...
  return (
    <main className="screen studio" data-expr={expr}>
      ...
      {dialog === 'photo' && (
        <PhotoDialog
          render={renderPhotoFor}
          filename={`${(session.modelName || face.nameEl).replace(/\s+/g, '-')}-makeup.png`}
          onDone={onPhotoDone}
